(function () {
    ...
    #content table.gauss-table th, #content table.gauss-table td { display: table-cell !important; vertical-align: middle !important; white-space: nowrap !important; border: 1px solid #007bff !important; background: #ffffff !important; padding: 8px 12px !important; border-radius: 8px !important; box-shadow: 0 0 4px rgba(0,123,255,0.18) !important; color: #0f0f23 !important; }
    #content table.gauss-table thead th { background-color: #007bff !important; color: white !important; font-weight: bold !important; padding: 6px 10px !important; border-radius: 6px !important; text-align: center !important; }
    #content table.gauss-table tbody tr:nth-child(even) td { background: #f0f8ff !important; }
    .solution-grid { display: flex; flex-wrap: wrap; gap: 15px; justify-content: center; margin-top: 15px; }
    .sol-box { border: 2px solid #007bff; border-radius: 10px; padding: 15px; min-width: 80px; text-align: center; background: #f8f9fa; box-shadow: 0 2px 4px rgba(0,123,255,0.2); }
    .sol-label { font-weight: bold; color: #007bff; margin-bottom: 5px; }
    .sol-value { font-family: 'Courier New', monospace; font-size: 1.1rem; color: #333; }
    `;

        const style = document.createElement("style");
        style.id = "doolittle-inline-styles";
    ...
    btn.addEventListener("click", async () => {
        ...
        try {
            const resp = await fetch("/eval/doolittle", {
                method: "POST",
            ...
            console.log(data)
            showMessage("Doolittle decomposition completed successfully.", "success");
